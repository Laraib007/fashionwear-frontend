...
export async function getServerSideProps(products) {
    ...
    let product = await Products.find({category: "hoodies"})
    let hood = {}
  for(let item of product){
  
    if(item.title in hood){
        if(!hood[item.title].color.includes(item.color) && item.avalibleQty > 0){
          hood[item.title].color.push(item.color)
        }
        if(!hood[item.title].size.includes(item.size) && item.avalibleQty > 0){
